    ...
    public class CommandManager
    {
        ...
        public bool ReceiveCommand(Command command)
        {
            try
            {
                ...
                {
                    case 213:
                        return LogicSetDoNotDisturb((LogicSetDoNotDisturbCommand)command);
                    case 215:
                    ...
                    case 530:
                        return LogicSetPlayerAgeReceived((LogicSetPlayerAgeCommand)command);
                    case 533:
                    ...
                    case 777:
                        return LogicSetTeamChatMuted((LogicSetTeamChatMutedCommand)command);
                    default:
        ...
        private bool LogicSetDoNotDisturb(LogicSetDoNotDisturbCommand command)
        {
            return true;
        }
        private bool LogicSetTeamChatMuted(LogicSetTeamChatMutedCommand comnand)
        {
            return true;
        }
        ...
        private bool LogicViewInboxNotificationReceived(LogicViewInboxNotificationCommand command)
        {
            ...
            if (HomeMode.Home.NotificationFactory.NotificationList[command.NotificationIndex].Id == 82)
            {
                Console.WriteLine("got the fucking notif 82");
            }
            if (HomeMode.Home.NotificationFactory.NotificationList[command.NotificationIndex].Id == 88)
            {
            ...
            if (HomeMode.Home.NotificationFactory.NotificationList[command.NotificationIndex].Id == 90)
            {
                LogicGiveDeliveryItemsCommand delivery = new LogicGiveDeliveryItemsCommand();
                if (HomeMode.Home.NotificationFactory.NotificationList[command.NotificationIndex].SkinID < 29000000){ 
                    DeliveryUnit unit = new DeliveryUnit(100);
                    ...
                    reward.PinGlobalId = (HomeMode.Home.NotificationFactory.NotificationList[command.NotificationIndex].SkinID - 29000000);
                    unit.AddDrop(reward);
                    delivery.DeliveryUnits.Add(unit);
                    delivery.Execute(HomeMode);
                }
            ...
            if (HomeMode.Home.NotificationFactory.NotificationList[command.NotificationIndex].Id == 93)
            {
   
                        CharacterData character = DataTables.Get(DataType.Character).GetDataWithId<CharacterData>(GlobalId.CreateGlobalId(16, HomeMode.Home.NotificationFactory.NotificationList[command.NotificationIndex].brawlerId));
            ...
            if (HomeMode.Home.NotificationFactory.NotificationList[command.NotificationIndex].Id == 95)
            {
                LogicGiveDeliveryItemsCommand delivery = new LogicGiveDeliveryItemsCommand();
                DeliveryUnit unit = new DeliveryUnit(100);
                GatchaDrop reward = new GatchaDrop(8);
        ...
        private bool LogicSelectStarPowerReceived(LogicSelectStarPowerCommand command)
        {
            ...
            {
                string[] cards = { "GrowBush", "Shield", "Heal", "Jump", "ShootAround", "DestroyPet", "PetSlam", "Slow", "Push", "Dash", "KSDBoost", "BurstHeal", "Spin", "Teleport", "Immunity", "Trail", "Totem", "Grab", "Swing", "Vision", "Regen", "HandGun", "Promote", "Sleep", "Slow", "Reload", "Reload", "Fake", "Trampoline", "Explode", "Blink", "PoisonTrigger", "Barrage", "Focus", "MineTrigger", "Reload", "Seeker", "Meteor", "HealPotion", "Stun", "TurretBuff", "StaticDamage" };
        ...
        private bool LogicSetPlayerAgeReceived(LogicSetPlayerAgeCommand command)
        {
            if (command.Age <= 14 || command.Age >= 45)
            {
                HomeMode.Home.SocialAge = 3;            
            }
            else
            {
                HomeMode.Home.SocialAge = 2;
            }
            return true;
        }
        ...
        private bool LogicPurchaseOfferReceived(LogicPurchaseOfferCommand command)
        {
            ...
            {
                Console.WriteLine($"Something happened wrong! {ex.Message}");
                return false;
            }            
        }
        private bool LogicSelectCharacterReceived(LogicSelectCharacterCommand command)
        {
            ...
            if (HomeMode.Avatar.HasHero(globalId))
            {
                ...
                if (HomeMode.Avatar.TeamId > 0)
                {
                    ...
                    {
                        string[] cards = { "GrowBush", "Shield", "Heal", "Jump", "ShootAround", "DestroyPet", "PetSlam", "Slow", "Push", "Dash", "KSDBoost", "BurstHeal", "Spin", "Teleport", "Immunity", "Trail", "Totem", "Grab", "Swing", "Vision", "Regen", "HandGun", "Promote", "Sleep", "Slow", "Reload", "Fake", "Trampoline", "Explode", "Blink", "PoisonTrigger", "Barrage", "Focus", "MineTrigger", "Reload", "Seeker", "Meteor", "HealPotion", "Stun", "TurretBuff", "StaticDamage" };
                        CharacterData cd = DataTables.Get(DataType.Character).GetDataByGlobalId<CharacterData>(hero.CharacterId);
                        CardData WildCard = null;
                        foreach (string cardname in cards)
                        {
        ...
        private bool LogicSetPlayerThumbnailReceived(LogicSetPlayerThumbnailCommand command)
        {
            try
            {
                ...
                if (command.ThumbnailInstanceId > 57)
                {
                ...
                bool IsCreator = HomeMode.Home.CreatorEndTime >= DateTime.UtcNow;
    
                if (HomeMode.Home.CreatorEndTime < DateTime.UtcNow && command.ThumbnailInstanceId == 57)
                {
                    HomeMode.Home.ThumbnailId = GlobalId.CreateGlobalId(28, 0);

                    if (Sessions.IsSessionActive(HomeMode.Avatar.AccountId))
                    {
                        Session session = Sessions.GetSession(HomeMode.Avatar.AccountId);
                        session.GameListener.SendTCPMessage(
                        new AuthenticationFailedMessage()
                        {
                            Message = "Доступно только для Pablo Creators!"
                        }
                    );
                        Sessions.Remove(HomeMode.Avatar.AccountId);
                    }
        ...
        private bool LogicSetPlayerNameColorReceived(LogicSetPlayerNameColorCommand command)
        {
            ...
            if (command.NameColorInstanceId > 24)
                {
                    AuthenticationFailedMessage loginFailed = new AuthenticationFailedMessage
                    {
                        ErrorCode = 1,
                        Message = "Обнаружен модифицированный клиент!"
                    };
                    Connection.Send(loginFailed);
                    return false;
                }
            
            bool isPremium = HomeMode.Home.PremiumEndTime >= DateTime.UtcNow;
            bool isCreator = HomeMode.Home.CreatorEndTime >= DateTime.UtcNow;
    
            if (HomeMode.Home.PremiumEndTime < DateTime.UtcNow && HomeMode.Home.CreatorEndTime < DateTime.UtcNow && command.NameColorInstanceId > 11)
        {       
            HomeMode.Home.NameColorId = GlobalId.CreateGlobalId(43, 0);
            ...
            if (Sessions.IsSessionActive(HomeMode.Avatar.AccountId))
        {
            ...
            session.GameListener.SendTCPMessage(
                new AuthenticationFailedMessage()
                {
                    Message = "Доступно только для Pablo Premium / Creators!"
                }
            );
            Sessions.Remove(HomeMode.Avatar.AccountId);
        }
