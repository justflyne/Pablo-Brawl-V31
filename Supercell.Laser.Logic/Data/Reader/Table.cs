    ...
    public class Table
    {
        ...
        public Table(string path)
        {
            ...
            using (var reader = new TextFieldParser(path))
            {
                ...
                if (columns == null)
                    throw new Exception("Header row is missing or invalid in CSV.");
                ...
                while (!reader.EndOfData)
                {
                    ...
                    // Проверяем, есть ли ключевое значение
                    ...
                    {
                        //Console.WriteLine($"[WARNING] Skipping row. Not enough columns: expected {_headers.Count}, got {values.Length}");
                        continue;
                    }
